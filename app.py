...
# ---------- Real index data (Dow, Nasdaq Composite, S&P 500) ----------
# Previously approximated via DIA/QQQ/SPY ETFs from the sentiment gauge's
# data source — QQQ specifically tracks the Nasdaq-100 (100 largest
# non-financial companies), not the actual Nasdaq Composite (~3,000+
# companies) that's meant when the news says "the Nasdaq." This fetches
# the real index values instead, via Twelve Data (already have a key).
# Free-tier access to index symbols isn't confirmed in advance — same
# defensive, error-surfacing approach as everything else this session.
INDICES_CACHE_SECONDS = 3 * 60  # NOT 60s — that would mean a continuous
    # 3-credits-per-minute draw against the same shared Twelve Data budget
    # that caused real rate-limit contention earlier this session. Index
    # values don't need minute-by-minute freshness to feel current.
INDICES_SYMBOLS = {"dow": "DJI", "nasdaq": "IXIC", "sp500": "GSPC"}
INDICES_LABELS = {"dow": "Dow Jones Industrial Average", "nasdaq": "Nasdaq Composite", "sp500": "S&P 500"}
...
def fetch_indices_all():
    ...
    out = {}
    try:
        acquire_twelvedata_credits(len(INDICES_SYMBOLS))
        symbols_param = ",".join(INDICES_SYMBOLS.values())
        r = twelvedata_get(
            "https://api.twelvedata.com/quote",
            {"symbol": symbols_param, "apikey": TWELVE_DATA_API_KEY},
        )
        r.raise_for_status()
        data = r.json()
        # Twelve Data returns a single object (not keyed by symbol) when
        # only one symbol is requested, and a dict keyed by symbol for
        # multiple — normalize both shapes.
        by_symbol = data if len(INDICES_SYMBOLS) > 1 and "symbol" not in data else {symbols_param: data}
        for key, symbol in INDICES_SYMBOLS.items():
            entry = by_symbol.get(symbol)
            if not entry or entry.get("status") == "error":
                out[key] = {"ok": False, "error": (entry or {}).get("message", "symbol missing from response")}
                continue
            try:
                out[key] = {
                    "ok": True,
                    "value": round(float(entry["close"]), 2),
                    "change": round(float(entry["change"]), 2) if entry.get("change") is not None else None,
                    "percent_change": round(float(entry["percent_change"]), 2) if entry.get("percent_change") is not None else None,
                }
            except (KeyError, ValueError, TypeError) as e:
                out[key] = {"ok": False, "error": f"unexpected shape: {e!r} — raw: {str(entry)[:150]}"}
        _indices_status["last_error"] = None
    except Exception as e:
        _indices_status["last_error"] = str(e)
        print(f"[indices] fetch failed: {e!r}", flush=True)
        raise
    _indices_cache["data"] = out
    _indices_cache["ts"] = time.time()
    print(f"[indices] updated: {out}", flush=True)


@app.route("/api/indices")
def indices():
    if not TWELVE_DATA_API_KEY:
        return jsonify({k: {"ok": False, "error": "TWELVE_DATA_API_KEY not set"} for k in INDICES_SYMBOLS})
    if _indices_cache["data"] is None:
        return jsonify({k: {"ok": False, "error": "not fetched yet"} for k in INDICES_SYMBOLS})
    return jsonify(_indices_cache["data"])


@app.route("/api/debug/indices")
def indices_debug():
    return jsonify({
        "twelve_data_key_set": bool(TWELVE_DATA_API_KEY),
        "symbols": INDICES_SYMBOLS,
        "cache_seconds_old": round(time.time() - _indices_cache["ts"], 1) if _indices_cache["data"] else None,
        "cached_data": _indices_cache["data"],
        "last_attempt_seconds_ago": round(time.time() - _indices_status["last_attempt"], 1) if _indices_status["last_attempt"] else None,
        "last_error": _indices_status["last_error"],
    })


def _indices_background_loop():
    print("[indices] background thread started", flush=True)
    time.sleep(3)  # first in the stagger queue — small, cheap request
    while True:
        if TWELVE_DATA_API_KEY:
            try:
                fetch_indices_all()
            except Exception as e:
                print(f"[indices] fetch_indices_all raised: {e!r}", flush=True)
        else:
            print("[indices] TWELVE_DATA_API_KEY not set, skipping", flush=True)
        time.sleep(INDICES_CACHE_SECONDS)


threading.Thread(target=_indices_background_loop, daemon=True).start()
